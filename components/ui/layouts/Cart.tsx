...
export default function Cart() {
  ...
  const handleRemoveBooking = async (bookingId: number) => {
    try {
      ...
      toast.success(result.message || "Booking removed from cart");
      refetch();
    } catch (error: any) {
      const errorMessage = error?.data?.message || "Failed to remove booking";
      toast.error(errorMessage);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          <ShoppingCart className="w-6 h-6 text-[#4157FE]" />
          <h2 className="text-2xl font-bold text-[#242565]">Your Cart</h2>
          {pendingBookings.length > 0 && (
            <Badge className="bg-[#4157FE] text-white">
              {pendingBookings.length}{" "}
              {pendingBookings.length === 1 ? "item" : "items"}
        ...
        <div className="space-y-6">
          ...
          <Alert className="border-orange-200 bg-orange-50">
            ...
            <AlertDescription className="text-orange-800 text-sm">
              <strong>Important:</strong> Bookings will expire after 24 hours if
              payment is not completed. Please complete your payment to confirm
              your seats.
            </AlertDescription>
          </Alert>
          ...
          {pendingBookings.map((booking: any) => {
            ...
            const quantity = booking.quantity || booking.seatsBooked || 0;
            const total = unitPrice * quantity;
                ...
                <CardContent className="p-4 md:p-6">
                  <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
                    ...
                    <div className="flex-1 space-y-3 md:space-y-4">
                      <div>
                        <div className="flex items-start justify-between gap-4 mb-2">
                          <h3 className="text-lg md:text-xl font-bold text-[#242565]">
                            {event.title}
                          </h3>
                          <Badge
                            variant="outline"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 text-xs md:text-sm text-[#8570AD]">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4" />
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-2">
                        <Button
                          onClick={() => handleProceedToPayment(booking.id)}
                          className="flex-1 bg-[#4157FE] hover:bg-[#7B8BFF] h-10 md:h-11 text-sm md:text-base"
                        >
                          <CreditCard className="w-4 h-4 mr-2" />
                          Proceed to Payment
                        </Button>
                        ...
                        >
                          <AlertDialogTrigger asChild>
                            <Button
                              variant="outline"
                              className="sm:w-auto border-red-200 text-red-600 hover:bg-red-50 h-10 md:h-11 text-sm md:text-base"
                            >
                              <Trash2 className="w-4 h-4 mr-2" />
                              Remove
                            </Button>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to remove "{event.title}"
                                from your cart? This will cancel your booking
                                and release the reserved seats.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>
